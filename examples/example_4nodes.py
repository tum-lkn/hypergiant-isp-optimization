...
mytopo = model.topology.Topology(
    name="MyTestTopology",
    parameter={
        constants.KEY_IP_LIGHTPATH_CAPACITY: 6
    }
)
...
mydemandset.append(
    model.demand.Hypergiant(
        name="alpha",
        peering_nodes=[
            model.demand.PeeringNode(nid=model.topology.NodeIdGenerator.get_next_id(), parent=mytopo.get_node_by_id(6),
                                     capacity=4),
            model.demand.PeeringNode(nid=model.topology.NodeIdGenerator.get_next_id(), parent=mytopo.get_node_by_id(8),
                                     capacity=6)
        ],
        user_nodes=[
            model.demand.EndUserNode(nid=model.topology.NodeIdGenerator.get_next_id(), parent=mytopo.get_node_by_id(5),
                                     demand_volume=3),
