...
if __name__ == '__main__':
    ...
    parser.add_argument("--time_agg", type=int, default=2)

    args = parser.parse_args()

    BASE_IN_FOLDER = f"{config.BASE_PATH}/input_{args.time_agg}h"
    BASE_OUT_FOLDER = f"{config.BASE_PATH}/output_{args.time_agg}h_fixed_shuffle_peering_and_times"
    ...
    for seed in range(0, 30):
        ...
        for folder_suffix in config.FOLDER_SUFFIX_DAY:
            fname_tuples = helpers.get_input_file_tuples(f"{BASE_IN_FOLDER}_{folder_suffix}_fixed_shuffle_timings/seed_{seed}").values()
        ...
        for (topo, dem), algo, fixed_layer in itertools.product(
            ...
            scenario_cfgs.append(
                scenario.ScenarioConfiguration(
                    ...
                    outputs=[output.file_writer.JsonWriterConfiguration(f"{BASE_OUT_FOLDER}/seed_{seed}")]
                )
            )
